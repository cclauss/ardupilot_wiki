    ...
    for key, value in PARAMETER_SITE.items():
        ...
        if key == 'AP_Periph':
            ...
            if site == key or site is None:
                ...
                if 'antennatracker' in key.lower():
                    # To main the original script approach instead of the
                    # build_parameters.py approach.
                    old_parameters_mask = (
                        os.getcwd()
                    ...
                    try:
                        ...
                        if not os.path.isfile(new_file):
                            ...
                            if ("parameters.rst" in filename) or (
                                ...
                                # It is different?  OR is this one the latest. |
                                # Latest file must be built every time in order to
                                # enable Sphinx create the correct references across
                                # the wiki.
...
%s
''' % (
            reference_for_board(platform_key),
            platform_key,
            underline,
            t,
        )
...
if __name__ == "__main__":
    ...
    if error_count > 0:
        ...
        for msg in error_log:
            print(
                f"\033[1;31m[update.py][error]: {msg}\033[0m", file=sys.stderr
            )  # noqa: E702,E231
