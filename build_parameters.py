...
def fetch_releases(firmware_url, vehicles):
    ...
    for f in vehicles:
        ...
        for folder in page_links:
            # Non clever way to filter the strings insert by makehtml.py, unwanted folders, and so.
    ...
    def fetch_commit_hash(version_link, board, file):
        ...
        try:
            ...
            if regex.search(vehicle) is None:  # there are some non standard names
                ...
                vehicle = vehicle_old_to_new_name[vehicle.strip()]
            else:
...
def generate_rst_files(commits_to_checkout_and_parse):
    ...
    def replace_anchors(source_file, dest_file, version_tag):
        ...
        for line in file_in:
            if (re.match("(^.. _)(.*):$", line)) and ("latest" not in version_tag):
                # renames the anchors, but leave latest anchors "as-is" to maintain
                # compatibility with all links across the wiki
            ...
            elif ("=======================" in line) and (not found_original_title):
    ...
    for i in commits_to_checkout_and_parse:
        ...
        if (
            "3.2.1" in version  # last stable APM Copter
        ...
        try:
            ...
            if os.path.exists("Parameters.rst"):
                ...
                # bunch of these, eg.
                # [build_parameters.py][error]: Parameters.rst not found to rename to
                # parameters-Copter-stable-V4.0.0.rst
...
# Present work dict.
print_versions(commits_to_checkout_and_parse)
